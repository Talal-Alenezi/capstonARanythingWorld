...
namespace PathCreationEditor {
    ...
    public class PathEditor : Editor {
            ...
            using (var check = new EditorGUI.ChangeCheckScope ()) {
                ...
                if (data.showPathOptions) {
                    ...
                    if (GUILayout.Button ("Reset Path")) {
                        ...
                        data.ResetBezierPath (new Vector3(0.0f,0.0f,0.0f), in2DEditorMode);
                        EditorApplication.QueuePlayerLoopUpdate ();
                    }
                    GUILayout.EndHorizontal();
                    GUILayout.Space (inspectorSectionSpacing);
                }
                data.showLoadPathOptions = EditorGUILayout.Foldout(data.showLoadPathOptions, new GUIContent("Load Path From File"), true, boldFoldoutStyle);
                if (data.showLoadPathOptions)
                {
                    creator.PathConfig = (PathConfig)EditorGUILayout.ObjectField("Path Config:", creator.PathConfig, typeof(PathConfig), true);
                    GUILayout.BeginHorizontal();
                    data.showScalePathTool = EditorGUILayout.BeginToggleGroup("Enable Path Scaling", data.showScalePathTool);
                    pathScalar = EditorGUILayout.FloatField(pathScalar);
                    GUILayout.EndHorizontal();
                    EditorGUILayout.EndToggleGroup();


                    #region Load Path From Config
                    GUILayout.BeginHorizontal();
                    if (GUILayout.Button("New"))
                    {
                        creator.PathConfig = creator.CreateNewConfigFromPath();
                        string itemID = System.Guid.NewGuid().ToString();
                        AssetDatabase.CreateAsset(creator.PathConfig, pathConfigDirectory + "PathConfig-" + itemID + ".asset");
                    ...
                    {
                        //Load scaled path
                        if (data.showScalePathTool)
                        {
                            creator.LoadPath(pathScalar);
                        }
                        else
                        {
                            creator.LoadPath();
                ...
                data.showSavePathOptions = EditorGUILayout.Foldout(data.showSavePathOptions, new GUIContent("Save Path To file"), true, boldFoldoutStyle);
                if (data.showSavePathOptions)
                {
                    GUILayout.Label("Save Path");
                    if (GUILayout.Button("Save To Current Path"))
                    {
                    ...
                    GUILayout.BeginHorizontal();
                    data.savedPathID = GUILayout.TextField(data.savedPathID, 100);
                    if(GUILayout.Button("Save to path"))
                    {
                        PathConfig newConfigInstance = creator.CreateNewConfigFromPath();
                        AssetDatabase.CreateAsset(creator.PathConfig, pathConfigDirectory + data.savedPathID + ".asset");
                        AssetDatabase.SaveAssets();
                        EditorApplication.QueuePlayerLoopUpdate();
                    }
                    GUILayout.EndHorizontal();
        ...
        void DrawHandle (int i) {
            ...
            float anchorHandleSize = GetHandleDiameter (pathScalar*creator.gameObject.transform.localScale.x * globalDisplaySettings.anchorSize * data.bezierHandleScale, bezierPath[i]);
            float controlHandleSize = GetHandleDiameter (pathScalar*creator.gameObject.transform.localScale.x* globalDisplaySettings.controlSize * data.bezierHandleScale, bezierPath[i]);
